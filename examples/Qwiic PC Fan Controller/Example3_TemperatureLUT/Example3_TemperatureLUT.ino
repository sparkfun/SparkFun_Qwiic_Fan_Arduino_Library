...
void setup()
{
  ...
  Serial.println("Qwiic PC Fan Controller Example 3: Thermal Fan Speed Control with TMP102 and LUT");
...
void loop() {
  ...
  if(idx == 0) // min
  {
    Fan.setPercentThrottle(LUT[0][1]);
  }
  else if(idx == sizeofLUT) // max
  {
    Fan.setPercentThrottle(LUT[idx-1][1]);
  }
  else // linear interpolate
  {
    ...
    Fan.setPercentThrottle(output); // Output is a float but we'll just let it get truncated for this demo
  }
